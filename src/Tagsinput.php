...
class Tagsinput{
  ...
  protected $addAcCallback = false;
    ...
    $this->ac = new Autocompleter($this->divid . '_ac', $label, $url, $this->options_ac);
	}
  ...
  public function printAcCallback(){
    ...
    if ($this->addAcCallback){
      ...
      $str .= '    jQuery(\'#' . $this->ulid . '\').data(\'tagsinput\').addtolist(data);' . PHP_EOL;
      $str .= '  }' . PHP_EOL;
    }
    return $str;
  }
  ...
  public function printTagInputInit(){
    ...
    $str .= '   jQuery(\'#' . $this->ulid  . '\').tagsinput({'. PHP_EOL;
  ...
  public function printReset(){
    return 'jQuery(\'#' . $this->ulid  . '\').data(\'tagsinput\').reset()';
  }

  public function printCount(){
    return 'jQuery(\'#' . $this->ulid  . '\').data(\'tagsinput\').count()';
  }

  public function printGetArrayValues(){
    return 'jQuery(\'#' . $this->ulid  . '\').data(\'tagsinput\').getArrayValues()';
  }

  public function printGetCommaSepValues(){
    return 'jQuery(\'#' . $this->ulid  . '\').data(\'tagsinput\').getCommaSepValues()';
  }

  public function printSerialize($champ){
    return 'jQuery(\'#' . $this->ulid  . '\').data(\'tagsinput\').serialize(\'' . $champ . '\')';
  }

  public function printAddToList($varia){
    return 'jQuery(\'#' . $this->ulid  . '\').data(\'tagsinput\').addtolist(' . $varia . ')';
  }
